...
**Purpose**: Close gaps between the current implementation and spec.md / plan.md / tasks.md / constitution found by `/speckit-converge` (2026-09-03). Tasks are ordered CRITICAL first. Evidence paths refer to the current package location `src/biosciences_mcp/` (plan.md still names `src/lifesciences_mcp/`).

- [ ] T073 CRITICAL: Add `slim: bool = False` parameter to `search_genes` in `src/biosciences_mcp/servers/entrez.py` and `EntrezClient.search_genes` in `src/biosciences_mcp/clients/entrez.py`; when `slim=True` return only `id`, `symbol`, `name`, `score` per data-model.md GeneSearchCandidate slim mode (lines 85-89), matching the `slim` parameter every sibling search tool exposes (`servers/hgnc.py:39`, `servers/uniprot.py:37`, `servers/ensembl.py:50`); add a unit test in `tests/unit/test_entrez_client.py` asserting slim items omit `description`/`organism` per Constitution IV (missing)
- [ ] T074 Populate `EntrezGene.description` in `get_gene`: extend `_parse_gene_xml` in `src/biosciences_mcp/clients/entrez.py` (return dict at lines 350-361 has no `description` key, so line 653 is always None) to extract other designations from `Entrezgene_prot/Prot-ref/Prot-ref_name/Prot-ref_name_E` (joined with "; ") and pass it through; add a unit test with sample XML asserting `description` is set per FR-011 / data-model.md:128-130 (partial)
- [ ] T075 Change the `search_genes` MCP tool default in `src/biosciences_mcp/servers/entrez.py:46` from `organism: str | None = "human"` to `organism: str | None = None` (all organisms) and update the docstring at lines 58-59, so the tool matches `EntrezClient.search_genes` (client already defaults None) per FR-003 and contracts/search_genes.yaml:26 (contradicts)
- [ ] T076 Return `UPSTREAM_ERROR` (not `ENTITY_NOT_FOUND`) when efetch XML cannot be parsed: in `src/biosciences_mcp/clients/entrez.py` distinguish "parse failure" from "no Entrezgene element" in `_parse_gene_xml` (lines 275-363; `except ET.ParseError` returns None, which `get_gene` lines 637-643 maps to ENTITY_NOT_FOUND), and also catch `defusedxml.DefusedXmlException` (a `ValueError`, not a `ParseError`, so it currently escapes as an unhandled exception); emit `UPSTREAM_ERROR` with a recovery_hint describing the malformed response; add a unit test feeding malformed XML and asserting `ErrorCode.UPSTREAM_ERROR` per spec Edge Case "XML parsing fails" and contracts/get_gene.yaml:127 (contradicts)
- [ ] T077 Handle an invalid `organism` filter with an error and guidance instead of a silent empty envelope: in `EntrezClient.search_genes` (`src/biosciences_mcp/clients/entrez.py:520-531`) inspect the esearch `esearchresult.errorlist.phrasesnotfound` / `warninglist` for the organism term and, when the organism phrase is not found, return `AMBIGUOUS_QUERY` (registry code) with `invalid_input=organism` and a recovery_hint listing the supported common names in `organism_map` (lines 177-185) plus "or a scientific name"; add a mocked unit test per spec Edge Case "organism filter is invalid" (partial)
- [ ] T078 Add runtime value validation to `EntrezCrossReferences` in `src/biosciences_mcp/models/entrez.py` (lines 98-178 have no validators and `extra: "allow"`): validate `hgnc` against `^HGNC:\d+$`, `ensembl_gene` against `^ENSG\d{11}$`, `omim` against `^\d{6}$` per contracts/get_gene.yaml:82-102 (reuse `CROSS_REF_PATTERNS` from `src/biosciences_mcp/models/cross_references.py:16-25` where keys match), dropping or rejecting non-conforming values so `INVALID_CROSS_REFERENCE` semantics hold; add unit tests in `tests/unit/test_entrez_models.py` per Constitution Required Pattern "Cross-reference regex validation" (partial)
- [ ] T079 Strengthen cross-reference integration assertions in `tests/integration/test_entrez_api.py`: `test_get_gene_tp53` (lines 151-155) must assert `hgnc == "HGNC:11998"`, `ensembl_gene == "ENSG00000141510"` and that `uniprot` is present, and `test_get_gene_cross_references` (lines 174-188) must assert HGNC/Ensembl/UniProt/RefSeq mappings as T030/T033 describe, so SC-003 is actually verified rather than only `len(xrefs) > 0` per T030, T033, SC-003 (partial)
- [ ] T080 Replace placeholder rate-limit/backoff/error-mapping tests with real mocked-HTTP tests in `tests/unit/test_entrez_client.py`: `test_backoff_delay_calculation` (lines 80-84) has no assertions and currently fails `uv run ruff check` (F841 unused `client` at line 82, so T066 is not satisfied); add tests that mock `httpx.AsyncClient.get` to return 429 then 200 and assert a retry occurred (FR-021), that a persistent 429 yields `ErrorCode.RATE_LIMITED` whose recovery_hint contains `NCBI_API_KEY` (FR-018, replacing the hand-built envelope in `tests/integration/test_entrez_api.py:318-333`), and that a 503 yields `ErrorCode.UPSTREAM_ERROR`; replace the `hasattr`-only `test_error_envelope_creation_method_exists` (lines 90-93) with these per T051, T061, T066 (partial)

> Note (2026-09-03, after baseline converge): PR #10 changed the parser to read `Gene-ref_db` first and emits registry-form `uniprot`/`ensembl_gene`; T078 (runtime validation) and T079 remain open but their expected values are now the registry form.
