...
- [ ] T034 CRITICAL: Apply runtime regex validation to HGNC-built cross-references — validate each value produced by `HGNCClient._build_cross_references` in src/biosciences_mcp/clients/hgnc.py against the ADR-001 Appendix A patterns (`CROSS_REF_PATTERNS` in src/biosciences_mcp/models/cross_references.py is defined but never applied; the model validator only strips empties) and surface failures via `INVALID_CROSS_REFERENCE` (currently never raised); add a unit test that a malformed value is rejected per Constitution Required Pattern "Cross-reference regex validation" (missing)
- [ ] T035 Honor `slim=True` in `HGNCClient.search_genes` (src/biosciences_mcp/clients/hgnc.py) so slim output is the id/name/score projection and differs from default mode — the parameter is accepted but never read, so both modes return an identical 4-field SearchCandidate; add a unit test asserting the two modes differ per FR-007 / US1/AC4 / Constitution IV (partial)
- [ ] T036 Normalize and URL-encode search queries in `HGNCClient.search_genes` and `_search_by_alias` (src/biosciences_mcp/clients/hgnc.py) — split slash-delimited inputs like "TP53/P53", search each variant, and merge results; currently the raw query is interpolated into the `/search/{query}` path so a slash is interpreted as an HGNC field selector, per spec Edge Case "Special characters" (missing)
- [ ] T037 Add tests/unit/test_hgnc_client.py with mocked HTTP responses (reuse the unused `mock_hgnc_search_response` / `mock_hgnc_fetch_response` fixtures in tests/conftest.py) covering: BRCA1 ranked first for query "BRCA1" (US1/AC1), slim vs default output (US1/AC4), `cross_references.uniprot` as a list of isoforms (US2/AC4), cursor decode/offset and `cursor=null` on the final page (US4/AC2-AC3), and ENTITY_NOT_FOUND on an empty docs array (US2/AC3) per plan: tests/unit structure (partial)
- [ ] T038 Return a dedicated short-query hint from `HGNCClient.search_genes` (src/biosciences_mcp/clients/hgnc.py:136-137) — the empty/short-query branch reuses `ErrorEnvelope.ambiguous_query`, whose message says "returned 0 results" and whose recovery_hint is "Refine query with more specific terms" instead of "provide at least 2 characters", per spec Edge Case "Empty query" and research.md Q8 (partial)
- [ ] T039 Propagate `Retry-After` into the RATE_LIMITED envelope and recommend exponential backoff in its recovery_hint in src/biosciences_mcp/clients/hgnc.py (`ErrorEnvelope.rate_limited()` is called without `retry_after` at the 429 branches; guard `int(retry_after)` against non-integer HTTP-date values in `_rate_limited_get`), and map a post-retry 403 to RATE_LIMITED rather than UPSTREAM_ERROR per spec Edge Case "Rate limiting" and research.md Q3/Q8 (partial)
- [ ] T040 Extend tests/integration/test_concurrency.py to submit 100 concurrent requests (currently 20 search + 10 get) and assert no RATE_LIMITED or exception results, so the load test actually verifies SC-007 as T033 intended (partial)
- [ ] T041 Reconcile specs/001-hgnc-mcp-server/contracts/hgnc-tools.json `CrossReferences` (7 keys, `additionalProperties: false`) with the keys `get_gene` actually emits (adds `ucsc`, `orphanet`, `pubmed` per ADR-001 Appendix A) and add a contract test that loads hgnc-tools.json and validates the search_genes/get_gene schemas against it, since no test currently reads the contract file per T029 / plan: contracts/hgnc-tools.json (partial)
