...
**Purpose**: Remaining work identified by `/speckit-converge` (2026-09-03) assessing the current checkout against spec.md, plan.md, tasks.md, and Constitution v1.1.0. Constitution items first, then HIGH/MEDIUM/LOW.

- [ ] T077 CRITICAL: Make `UniProtClient._map_cross_references()` in src/biosciences_mcp/clients/uniprot.py:114-168 emit registry-conformant values: strip version suffixes from Ensembl transcript IDs (`ENST00000269305.9` → `ENST00000269305`), map RefSeq via the `NucleotideSequenceId` property to versionless `NM_`/`NR_` IDs (registry `^[NX][MR]_\d+$`), emit a single 6-digit `omim` (select the MIM entry whose `Type` property is `gene`) instead of a comma-joined string, and emit `orphanet` as `ORPHA:<id>` (single value); then delete the `uniprot.get_protein` entry from `REGISTRY_DEVIATIONS` in tests/contract/test_wire_contracts.py:38-42 so the contract test enforces conformance per Constitution III, FR-005, FR-006 (contradicts)
- [ ] T078 CRITICAL: Implement the `slim` parameter in `UniProtClient.search_proteins()` (src/biosciences_mcp/clients/uniprot.py:170-287; accepted at :173 but never read) so `slim=True` returns only `id`, `name`, `score` per candidate (omit `organism`/`gene_names`), matching the server docstring at src/biosciences_mcp/servers/uniprot.py:48 per Constitution IV, FR-008 (partial)
- [ ] T079 CRITICAL: Fix cursor pagination in `UniProtClient.search_proteins()`: parse the next cursor from the `Link: <...cursor=...>; rel="next"` response header and `total_count` from the `X-Total-Results` header instead of `data.get("cursor")` at src/biosciences_mcp/clients/uniprot.py:279-287 (UniProt's JSON body contains only `results`, so the cursor is always null and page 2 is unreachable); then make tests/integration/test_uniprot_api.py::test_search_proteins_pagination (:53-78) assert a non-null cursor for "kinase" rather than skipping when absent per US1/AC2, FR-007 (missing)
- [ ] T080 Populate `cross_references.ensembl_gene` in `_map_cross_references()` (src/biosciences_mcp/clients/uniprot.py:137-138) by extracting the versionless `GeneId` property (`ENSG...`) from UniProt `Ensembl` cross-references, as documented in specs/002-uniprot-mcp-server/research.md R5 and shown in plan.md D3 quickstart (`cross_references.ensembl_gene`) per FR-005, US3/AC1 (partial)
- [ ] T081 Return `RATE_LIMITED` (with Retry-After seconds in the hint) for HTTP 429/403 in `UniProtClient.get_protein()` src/biosciences_mcp/clients/uniprot.py:350-386, which currently falls through to `UPSTREAM_ERROR` ("UniProt API error: 429") because `raise_for_status()` routes 429 into the generic branch per FR-009 (partial)
- [ ] T082 Implement human-protein prioritization in `UniProtClient.search_proteins()` (src/biosciences_mcp/clients/uniprot.py:204-276) via an organism boost (e.g. `organism_id:9606` OR-clause, or post-ranking with a taxon-9606 score bonus before position decay) as planned in plan.md "Challenge 1" and research.md R2, and tighten tests/integration/test_uniprot_api.py::test_search_proteins_ranking (:341-367) to assert a Homo sapiens result is in the top 3 for "actin" per US1/AC4, FR-013 (partial)
- [ ] T083 Replace the HGNC-worded factories `ErrorEnvelope.rate_limited()`/`upstream_error()` used by `search_proteins()` at src/biosciences_mcp/clients/uniprot.py:221,223,234 (they produce "HGNC API rate limit exceeded." / "HGNC API returned error N." from src/biosciences_mcp/models/envelopes.py:91-106) with UniProt-specific envelopes whose `RATE_LIMITED` hint includes the `Retry-After` value and suggests batch operations, per the research.md R7 hint table per US4/AC2, FR-010 (contradicts)
- [ ] T084 Implement the `AMBIGUOUS_QUERY` guard for short queries with >100 results in `UniProtClient.search_proteins()` using the unused `AMBIGUOUS_THRESHOLD` constant (src/biosciences_mcp/clients/uniprot.py:43) and the `X-Total-Results` header (e.g. query length <4 and total > threshold → `ErrorEnvelope.ambiguous_query()` with a refine hint), while keeping longer queries such as "kinase" paginating per US1/AC2 per US4/AC1 (missing)
- [ ] T085 Add a UniProt 100-concurrent-request test (asyncio.gather of `search_proteins`/`get_protein`, asserting no ErrorEnvelopes and rate-limiter spacing) to tests/integration/test_concurrency.py, which currently exercises only `HGNCClient` (:30-125) while tests/integration/test_performance.py::test_concurrent_search_performance (:137-170) runs 10 requests per SC-003, FR-014, plan touch-point tests/integration/test_concurrency.py (partial)
- [ ] T086 Create tests/unit/test_protein_models.py (marked `unit`, no network) covering `Protein.validate_curie_format` (src/biosciences_mcp/models/protein.py:69-92: accepts `UniProtKB:P04637`/`UniProtKB:A0A123B4C5`, rejects `P04637`, lowercase, wrong prefix), `ProteinSearchCandidate` score bounds (:34-42), omit-None serialization, and a mocked `_map_cross_references()` round-trip validated against tests/contract/registry.py patterns per plan touch-point tests/unit/test_protein_models.py, T012, T013 (missing)
- [ ] T087 Create specs/002-uniprot-mcp-server/data-model.md (Protein, ProteinSearchCandidate, validation rules per plan.md D1) and specs/002-uniprot-mcp-server/contracts/uniprot.openapi.yaml (search_proteins/get_protein contracts per plan.md D2), which plan.md Phase 1 lists as outputs but are absent from the feature directory per plan: D1 data model, D2 API contracts (missing)
- [ ] T088 Align `page_size` bounds with the plan.md D2 contract (`minimum: 1, maximum: 100, default: 50`): either lower `MAX_PAGE_SIZE` at src/biosciences_mcp/clients/uniprot.py:46 (currently 500) and add `ge=1, le=100` on the `search_proteins` tool parameter in src/biosciences_mcp/servers/uniprot.py:39, or document the 500 justification per Constitution IV ("never exceed without explicit justification") per plan: D2 search_proteins contract (contradicts)
- [ ] T089 Handle obsolete/merged UniProt accessions in `UniProtClient.get_protein()`: `LifeSciencesClient` (src/biosciences_mcp/clients/base.py:59-64) does not set `follow_redirects`, so a 3xx redirect to the primary accession raises via `raise_for_status()` (src/biosciences_mcp/clients/uniprot.py:349) and is reported as `UPSTREAM_ERROR` (:378-386); follow the redirect (or pass `follow_redirects=True` on the request) and surface the canonical accession per spec Edge Cases ("obsolete UniProt IDs that have been merged") (partial)
- [ ] T090 Harden `Retry-After` parsing in `UniProtClient._rate_limited_get()` (src/biosciences_mcp/clients/uniprot.py:94-95): `int(retry_after)` raises `ValueError` on the HTTP-date form, which `search_proteins` then reports as UPSTREAM_ERROR "Failed to parse UniProt API response" (:305-312); parse both delta-seconds and HTTP-date, falling back to `2**attempt` per FR-012 (partial)

> Note (2026-09-03, after baseline converge): T077 and T080 are closed by PR #10 (registry-form cross-references, `ensembl_gene` from the Ensembl `GeneId` property). T079 is tracked as AGE-705.
