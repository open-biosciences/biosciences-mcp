...
**Purpose**: Close gaps found by `/speckit-converge` between the current code and spec.md, plan.md, tasks.md, and the constitution (constitution items first, described as CRITICAL)

- [ ] T274 CRITICAL: Restructure `_rate_limited_get` in `src/biosciences_mcp/clients/iuphar.py` so backoff sleeps and retry requests happen outside the held `asyncio.Lock` (the recursive calls at lines 72/81 run inside `async with self._lock` at line 55; the lock is non-reentrant, so any 429/503/timeout hangs the client forever - follow the `clients/hgnc.py:74-94` pattern) per Constitution Required Pattern "Client-side Rate Limiting" / FR-023 (contradicts)
- [ ] T275 CRITICAL: Remove `Field(pattern=r"^IUPHAR:\d+$")` from the `iuphar_id` parameter of `get_ligand` and `get_target` in `src/biosciences_mcp/servers/iuphar.py` (lines 144, 307) so a raw string reaches the client's UNRESOLVED_ENTITY ErrorEnvelope with recovery_hint instead of a pydantic validation string, then delete the two `iuphar.*` entries from `RAW_STRING_DEVIATIONS` in `tests/contract/test_wire_contracts.py` per Constitution II failure mode / FR-006 / FR-013 / US2/AC2 / US4/AC2 (contradicts)
- [ ] T276 CRITICAL: Emit `chembl` cross-references in registry form `^CHEMBL\d+$` in `_map_ligand_cross_references` (`src/biosciences_mcp/clients/iuphar.py:127`) and `_map_target_cross_references` (`:173`) instead of stripping the prefix; update expectations in `tests/unit/test_iuphar_client.py::test_chembl_cross_reference_normalization` and `::test_drugbank_id_format_validation`, and delete the `iuphar.get_ligand` / `iuphar.get_target` entries from `REGISTRY_DEVIATIONS` in `tests/contract/test_wire_contracts.py` per Constitution III / ADR-001 Appendix A / FR-017 (contradicts)
- [ ] T277 CRITICAL: Return a Canonical ErrorEnvelope on every failure path in `src/biosciences_mcp/clients/iuphar.py`: map non-200 upstream statuses (currently `raise ValueError` at lines 277-278, 390-391, 522-523, 616-617 and re-raised at 463/688) and exhausted network retries (line 82) to RATE_LIMITED / UPSTREAM_ERROR via `_handle_http_error`, and return an envelope instead of `ValueError` for the `len(query) < 2` guard (lines 331-332, 551-552), so agents receive the RATE_LIMITED / UPSTREAM_ERROR hints the spec promises per FR-019 / FR-020 / US5/AC3 / spec Edge Cases / Constitution III (partial)
- [ ] T278 Add offline unit tests for the rate limiter in `tests/unit/test_iuphar_client.py` using a mocked `_get`: 1 req/s spacing, concurrent callers without races, exponential backoff on 429/503 that completes under a timeout (regression guard for T274), and thundering-herd re-check per T234-T237 / FR-022 / FR-023 / FR-024 / SC-002 (missing)
- [ ] T279 Populate `Target.gene_symbol` from a GtoPdb source that carries it (the `/targets/{id}` payload documented in plan.md R8 has no `geneSymbol`, so `src/biosciences_mcp/clients/iuphar.py:704` yields None; e.g. the human entry of `/targets/{id}/geneProteinInformation`) and make `tests/integration/test_iuphar_api.py::test_get_target_gene_symbol_populated` (line 497) assert unconditionally per FR-016 / US4/AC1 (partial)
- [ ] T280 Map HGNC identifiers into `cross_references.hgnc` (`HGNC:\d+`) in `_map_target_cross_references` (`src/biosciences_mcp/clients/iuphar.py:135-188` has no HGNC branch) so target records carry the UniProt/Ensembl/HGNC set promised by the spec and the `get_target` docstring (`src/biosciences_mcp/servers/iuphar.py:341`); if GtoPdb exposes no HGNC link, correct the docstring and quickstart instead per FR-017 / T068 (partial)
- [ ] T281 Reconcile Constitution IV for `search_ligands` / `search_targets` in `src/biosciences_mcp/servers/iuphar.py`: either add a `slim` parameter consistent with the hgnc/ensembl/drugbank search tools, or record the "search candidates are always slim" justification in the plan.md Complexity Tracking section (plan.md Constitution Check claims "Implements slim=True mode for search results", which the code does not have) per Constitution IV / plan: Constitution Check (partial)
- [ ] T282 Add the deferred Phase 8 coverage to `tests/integration/test_iuphar_api.py`: cross-reference format validation (T238-T243), remaining edge cases (T246-T251), schema/enum compliance for ligand types and target families (T252-T257, supports SC-007/SC-008), and the end-to-end workflows (T268-T270) per T238-T243, T246-T257, T268-T270 (missing)
- [ ] T283 Implement an AMBIGUOUS_QUERY ErrorEnvelope with a refine-query recovery hint for overly broad `search_ligands` queries in `src/biosciences_mcp/clients/iuphar.py` (no `AMBIGUOUS_QUERY` reference exists), or record the decision that broad queries are served through pagination per T212 / T213 / FR-020 (missing)
