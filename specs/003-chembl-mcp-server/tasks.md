...
**Purpose**: Close gaps found by `/speckit-converge` between spec.md/plan.md/tasks.md/constitution and the current code (package is `src/biosciences_mcp/`, formerly `src/lifesciences_mcp/`)

- [ ] T047 CRITICAL: Make the UNRESOLVED_ENTITY `recovery_hint` in `_validate_chembl_curie()` (src/biosciences_mcp/clients/chembl.py:195) point to the resolve tool, e.g. "Invalid ChEMBL CURIE format. Call search_compounds to resolve the identifier, then use 'CHEMBL:NNNNN' (e.g., 'CHEMBL:25')"; keep the format guidance so tests/unit/test_chembl_client.py:35 still passes and add an assertion that the hint names `search_compounds` per Constitution II (contradicts)
- [ ] T048 CRITICAL: Add runtime regex validation for every `cross_references` value emitted by ChEMBL — uniprot `^UniProtKB:[A-Z][A-Z0-9]{5,9}$`, drugbank `^DB:[0-9]{5}$`, pdb `^[0-9][A-Z0-9]{3}$`, pubchem_compound `^[0-9]+$`, chembl `^CHEMBL:[0-9]+$` (patterns from data-model.md L226-232) — as a validator on `Compound.cross_references` in src/biosciences_mcp/models/compound.py:139-142 or in `_build_cross_references()` (src/biosciences_mcp/clients/chembl.py:308-351); drop non-conforming values with a logged warning and reserve `ErrorCode.INVALID_CROSS_REFERENCE` (currently never emitted) for malformed xrefs per FR-024; add unit tests in tests/unit/test_chembl_models.py per Constitution Required Patterns (Cross-reference regex validation) (missing)
- [ ] T049 Populate `Compound.canonical_name` in full mode instead of hard-coding `None` in `_transform_to_compound()` (src/biosciences_mcp/clients/chembl.py:453) — derive from ChEMBL data (e.g. a systematic/IUPAC entry in `molecule_synonyms`, falling back to `pref_name`) so the key is present on the wire (OmitNoneModel currently drops it); add a unit test and an assertion in tests/integration/test_chembl_api.py `test_get_compound_aspirin_complete_record` per FR-010, FR-027 (partial)
- [ ] T050 Populate `cross_references.uniprot` with `UniProtKB:` CURIEs for known protein targets in full mode — the molecule endpoint's `cross_references` array (src/biosciences_mcp/clients/chembl.py:64-69, 328-351) carries no target xrefs, so source them from ChEMBL mechanism/target data (e.g. `new_client.mechanism.filter(molecule_chembl_id=...)` → `target_chembl_id` → target component accessions) via `_sdk_call_with_backoff()`; then restore the T027 assertion that CHEMBL:25 has `uniprot`, `pdb`, and `pubchem_compound` keys in tests/integration/test_chembl_api.py:265-274 (currently asserts only the `chembl` self-reference) per FR-017, US2/AC2, US3/AC1 (partial)
- [ ] T051 Add a cross-database resolution integration test in tests/integration/test_chembl_api.py that takes `cross_references` from `get_compound("CHEMBL:25")` and resolves at least one `uniprot` value via `UniProtClient.get_protein` and one `pubchem_compound` value via `PubChemClient.get_compound` (both clients exist in src/biosciences_mcp/clients/), replacing the prefix-only checks at tests/integration/test_chembl_api.py:276-300 per FR-031 (partial)
- [ ] T052 Reconcile the ChEMBL performance tests with their success criteria in tests/integration/test_performance.py: `test_get_compound_sc002` (L236-278) asserts max<15s/avg<5s instead of SC-002's <1s, and `test_batch_faster_than_sequential` (L311-341) asserts batch is "not >3x+2s slower" for 3 items instead of SC-007's >70% faster for 10 — either meet the criteria (e.g. warm-up request, skip the extra `drug_indication` call in the timed path) or record the relaxed thresholds as an explicit, justified deviation in the test docstrings and plan.md Complexity Tracking per SC-002, SC-007, T038, T039 (partial)
- [ ] T053 Add a ChEMBL concurrency test in tests/integration/test_performance.py that issues 100 concurrent `search_compounds` calls via `asyncio.gather`, asserts zero `ErrorEnvelope` results and that aggregate throughput stays consistent with the 10 req/s limiter (no 429 loops, no unbounded latency growth), mirroring `test_concurrent_search_performance` for UniProt (L137) per SC-004 and the Phase 7 header (tasks.md L284) (missing)
- [ ] T054 Surface the actual backoff duration in the RATE_LIMITED envelope: replace the hard-coded "Wait 60s before retrying" in `_map_sdk_error()` (src/biosciences_mcp/clients/chembl.py:231) with the computed delay (or `Retry-After` when the SDK exposes it) formatted as "Wait {seconds}s before retrying", and assert the number in tests/unit/test_chembl_client.py per US4/AC2, T034 (partial)
- [ ] T055 Review and justify or remove fields and calls not called for by spec.md, data-model.md, or contracts/: `CompoundSearchCandidate.is_parent` (src/biosciences_mcp/models/compound.py:51-54, clients/chembl.py:290-305), `Compound.max_phase` and `Compound.indications` (models/compound.py:119-127) with the extra `drug_indication` SDK call on every full-mode `get_compound` (clients/chembl.py:579-596, doubling upstream calls against SC-002), and tests/integration/test_chembl_timeout.py — if kept, add them to data-model.md, the contracts, and the spec Key Entities via the SpecKit workflow per plan: data model / contracts (unrequested)

> Note (2026-09-03, after baseline converge): T050 is superseded by PR #10: the registry form for `uniprot` is the bare accession, not a `UniProtKB:` CURIE, and `Compound.cross_references` is now the shared `CrossReferences` type. T048 (runtime regex validation) remains open.
