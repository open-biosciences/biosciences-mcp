...
**Purpose**: Remaining work identified by `/speckit-converge` on 2026-09-03 after assessing `src/biosciences_mcp/{clients,models,servers}/pubchem.py` and `tests/{unit,integration}/test_pubchem_*.py` against spec.md, plan.md, tasks.md, and the constitution. Ordered CRITICAL/HIGH first.

- [ ] T170 CRITICAL: Update the UNRESOLVED_ENTITY `recovery_hint` in `_validate_pubchem_curie()` (currently "…CID must be numeric.") and in the HTTP 400 branch of `_map_http_error()` in `src/biosciences_mcp/clients/pubchem.py` to point to the resolve tool, matching contracts/get_compound.json: "Use format 'PubChem:CID{number}' (e.g., 'PubChem:CID2244'). Call search_compounds to find valid CIDs."; add assertions that "search_compounds" appears in the hint to `TestCurieValidationInvalid` and `TestErrorMappingHTTP400` in `tests/unit/test_pubchem_client.py` and `test_get_compound_invalid_curie` in `tests/integration/test_pubchem_api.py` per Constitution II, US2/AC2, US4/AC1, FR-016 (contradicts)
- [ ] T171 Extend `_request_with_backoff()` in `src/biosciences_mcp/clients/pubchem.py` to apply exponential backoff on HTTP 429 in addition to 503, and add an explicit 429 → RATE_LIMITED branch in `_map_http_error()` (currently falls through to UPSTREAM_ERROR); add unit tests `test_exponential_backoff_429` and `test_error_mapping_429` in `tests/unit/test_pubchem_client.py` per FR-019 and spec Edge Cases (partial)
- [ ] T172 Run cross-reference extraction over the full synonym list, not the 20-entry truncation: make `_get_compound_synonyms()` return all synonyms (or return both full and truncated), call `_extract_cross_references()` on the full list in `get_compound()`, and truncate only the `synonyms` field to 20 in `src/biosciences_mcp/clients/pubchem.py`; then make `test_get_compound_cross_references_chembl` and `test_get_compound_cross_references_drugbank` in `tests/integration/test_pubchem_api.py` assert `"CHEMBL:25" in cross_references["chembl"]` and `"DB00945" in cross_references["drugbank"]` unconditionally per US3/AC1, US3/AC2, SC-003, FR-011 (partial)
- [ ] T173 Write unit tests with mocked responses in `tests/unit/test_pubchem_client.py` for `_extract_cross_references()` (`test_extract_chembl_from_synonyms`, `test_extract_drugbank_from_synonyms`, `test_extract_uniprot_from_xrefs`, `test_omit_if_null_pattern`, `test_cross_reference_format_chembl`, `test_cross_reference_format_uniprot`), `_get_compound_xrefs()`, `_get_compound_properties()` (`test_get_properties_aspirin`), and `get_compound()` end-to-end field mapping (`test_field_mapping_pubchem_to_model`) per tasks T101, T103, T123–T128 (missing)
- [ ] T174 Add runtime regex validation for `cross_references` values on `PubChemCompound` in `src/biosciences_mcp/models/pubchem_compound.py` (a `@field_validator("cross_references")` that rejects keys outside the 22-key registry and validates each value against the per-key patterns in `src/biosciences_mcp/models/cross_references.py`), with `test_compound_cross_references_regex_validation` in `tests/unit/test_pubchem_models.py` per Constitution Required Patterns: cross-reference regex validation (partial)
- [ ] T175 Make slim-mode `get_compound()` emit exactly `{id, name, molecular_formula}`: return `compound.to_slim()` (a dict) instead of `PubChemCompound(**compound.to_slim())` in `src/biosciences_mcp/clients/pubchem.py`, pass dicts through in the `get_compound` tool in `src/biosciences_mcp/servers/pubchem.py` (as plan.md Server Architecture shows), and assert `set(result_dict) == {"id", "name", "molecular_formula"}` in `test_get_compound_aspirin_slim` in `tests/integration/test_pubchem_api.py` per contracts/get_compound.json slim example and plan: Server Architecture (partial)
- [ ] T176 Align `search_compounds` slim semantics with the artifacts: change the server tool default in `src/biosciences_mcp/servers/pubchem.py` from `slim=True` to `slim=False` (contracts/search_compounds.json default false; plan.md Server Architecture), and either implement slim on `PubChemSearchCandidate` output (id, name, score only per Constitution IV) or record in contracts/search_compounds.json that candidates are already slim-sized so the flag is a no-op, per plan: Tool Contracts and Constitution IV (partial)
- [ ] T177 Populate `invalid_input` on the timeout and network-error `ErrorEnvelope`s in `_get_compound_properties()` in `src/biosciences_mcp/clients/pubchem.py` (pass the CID list string) so `search_compounds` never returns `"invalid_input": null` per FR-014 and T138 (partial)

> Note (2026-09-03, after baseline converge): T173 is closed by PR #10 (tests/unit/test_pubchem_client.py::TestCrossReferenceRegistryForm); `PubChemCompound.cross_references` is now the shared `CrossReferences` type. T174 remains open.
