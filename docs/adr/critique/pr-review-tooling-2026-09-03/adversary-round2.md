...
3. **ADR-005** — it lives at `biosciences-mcp/docs/adr/accepted/adr-005-v1.0.md` (CLAUDE.md's `biosciences-program/docs/adr/accepted/` does not exist). It makes `.worktrees/<name>` the canonical home for per-agent feature worktrees (lines 59, 108-113) and says nothing about removal. So the tool's default root sits inside ADR-sanctioned, actively used space — that is the cleanup adjacency. "Local mutation, never origin" is compatible with ADR-005's shared-history model; force-removal is not something ADR-005 contemplated. Recommend default root outside `.worktrees/` (any path works for git) or a manifest-scoped cleanup.
...
## 4. Reference spot-check (12 checked, live fetch or saved page dump)
